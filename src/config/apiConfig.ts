...
/**
 * Spreadsheet ID の表示時の最大文字数
 *
 * 理由: セキュリティ上、完全なIDを表示せず、先頭10文字のみ表示。
 */
export const SPREADSHEET_ID_DISPLAY_LENGTH = 10;

/**
 * 監査ログの値の最大表示文字数
 *
 * 理由: ログが肥大化するのを防ぐため、長い値は200文字で切り詰め。
 */
export const AUDIT_LOG_VALUE_MAX_LENGTH = 200;

/**
 * リポジトリ名の最大文字数（シート名用）
 *
 * 理由: Googleスプレッドシートのシート名は100文字まで。
 * リポジトリ名が長い場合は切り詰める。
 */
export const REPOSITORY_NAME_MAX_LENGTH = 100;

/**
 * GitHub App 秘密鍵のプレビュー表示文字数
 *
 * 理由: セキュリティ上、秘密鍵の先頭100文字のみ表示。
 */
export const PRIVATE_KEY_PREVIEW_LENGTH = 100;

/**
 * 数値の小数点以下の桁数（丸め精度）
 *
 * 理由: DORA指標などの計算結果を小数点第1位まで表示。
 * Math.round(value * 10) / 10 で実現。
 */
export const DECIMAL_PRECISION_MULTIPLIER = 10;
